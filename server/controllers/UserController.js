...
export const getUsers = async (req, res) => {
    const response = await Users.find().select({"username":1, "password":1});
...
export const Register = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ msg: "Register User Berhasil" });
    }
    catch (error) {
...
export const Login = async (req, res) => {
    try {
        ...
        if(user.length === 0) return res.status(404).json({msg: "User Tidak Ditemukan"});
